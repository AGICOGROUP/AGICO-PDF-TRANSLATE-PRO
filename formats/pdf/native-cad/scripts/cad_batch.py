...
def page_ocr(page, source_hash, cache, engine=None):
    ...
    key = hashlib.sha256(f'{source_hash}:full-page-v1:{page.number}:{scale}'.encode()).hexdigest()
    path = cache / (key+'.json')
    if path.exists():
        try:
...
def review_bundle(source, destination, records, candidate=None, candidate_hash=None, residual_script=None):
    ...
    binding = hashlib.sha256(json.dumps([REVIEW_CACHE_VERSION, source_hash, actual_candidate_hash, candidate_hash,
                                        records, residual_script], sort_keys=True).encode()).hexdigest()
    report_path = destination/'review-report.json'
    if report_path.exists():
        try:
            cached = json.loads(report_path.read_text(encoding='utf-8'))
            if cached.get('binding') == binding and cached.get('artifacts') and all(
                (destination/name).is_file() and hashlib.sha256((destination/name).read_bytes()).hexdigest() == digest
                for name,digest in cached['artifacts'].items()):
                return dict(cached, cache_hit=True, seconds=round(time.perf_counter()-started,3))
        except (ValueError, OSError):
            pass
        ...
        try:
            ...
            for page_index, page in enumerate(original):
                ...
                for number, record in enumerate(records):
                    if record.get('page',0) != page_index:
                        continue
                    rect = pymupdf.Rect(record['bbox'])
    ...
    artifacts = list(destination.glob('*.png')) + [destination/'index.html']
